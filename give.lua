function HandleGiveCommand(Split, Player)
	...
	local function Callback(newPlayer)
		if not Player then
			Player:SendMessage( cChatColor.Rose .. "[INFO]" .. cChatColor.White .. "Invalid Player")
			return true;
		end
	end
	...
	-- Function to do if player exists.
	local function Callback(newPlayer)
		...
		if (ItemsGiven == ItemAmount) then
			...
			newPlayer:SendMessage(cChatColor.Rose .. "[INFO] " .. cChatColor.White .. "Not enough space in inventory, only gave " .. ItemsGiven);
